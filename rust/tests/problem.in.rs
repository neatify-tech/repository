async fn execute_tool(
    ...
    let params = arguments
        .as_object()
        .ok_or_else(|| ProtocolError::new(-32602, "arguments must be an object"))?;
    ...
    let result = match name {
        ...
        "search_files" => run_tool(
            ...
            || async {
                ...
                let root_param = args.get("root").and_then(Value::as_str).unwrap_or(".");
        ...
        "find_files" => run_tool(
            ...
            || async {
                ...
                let file_type = args.get("type").and_then(Value::as_str).map(|value| value.to_string());
                let max_depth = args
                    .get("max_depth")
                    .and_then(Value::as_u64)
                    .map(|value| value as usize);
                let follow = args.get("follow").and_then(Value::as_bool).unwrap_or(false);
                let glob = args.get("glob").and_then(Value::as_bool).unwrap_or(false);
                let case_sensitive = parse_case_sensitivity(args.get("case_sensitive"))?;
                let exclude = args
                    .get("exclude")
                    .and_then(Value::as_array)
        ...
        "read_file" => run_tool(
            ...
            None,
            || async {
                let path = args.get("path").and_then(Value::as_str).ok_or_else(|| anyhow!("path is required"))?;
                ...
                if highlight {
                    let raw = tokio::fs::read_to_string(&resolved)
                        .await
                        .map_err(|err| format_io_error("read", &rel_path, err.into()))?;
                    let (lines, count, total, truncated, truncated_reason, long_lines) =
                        fs::format_line_slices(&raw, start_line, limit, max_total, max_line);
                    ...
                    let data = fs::read_file(&resolved, start_line, limit, max_total, max_line)
                        .await
                        .map_err(|err| format_io_error("read", &rel_path, err))?;
                    Ok(json!({
                        "path": rel_path,
        ...
        "read_multiple_files" => run_tool(
            ...
            || async {
                let paths = args
                    .get("paths")
                    .ok_or_else(|| anyhow!("paths is required"))?
                    .as_array()
        ...
        "move_file" => run_tool(
            ...
            || async {
                ...
                fs::move_path(&resolved_from, &resolved_to)
                    .await
                    .map_err(|err| format_io_error("move", &rel_from, err))?;
                Ok(json!({ "from": rel_from, "to": rel_to }))
            },
        )
        .await,
        "delete_file" => run_tool(
            ...
            || async {
                ...
                let rel_path = relative_to_root(&config.root, &resolved);
                fs::delete_path(&resolved)
                    .await
                    .map_err(|err| format_io_error("delete", &rel_path, err))?;
                Ok(json!({ "path": rel_path }))
            },
        )
        .await,
        "write_file" => run_tool(
            ...
            || async {
                ...
                let rel_path = relative_to_root(&config.root, &resolved);
                let data = fs::write_file(&resolved, content, mode, apply)
                    .await
                    .map_err(|err| format_io_error("write", &rel_path, err))?;
                let mut structured = json!({
                    "path": rel_path
                });
                if preview {
                    let before = data.get("before").and_then(Value::as_str).unwrap_or("");
                    let after = data.get("after").and_then(Value::as_str).unwrap_or("");
                    ...
                    let entry = PreviewEntry {
                        uri: review_uri,
                        html,
                        diff: data.get("diff").and_then(Value::as_str).unwrap_or("").to_string(),
                        path: structured
                            .get("path")
                            .and_then(Value::as_str)
                            .unwrap_or("file")
